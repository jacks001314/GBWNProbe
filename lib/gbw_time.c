...
int gbw_time_ansi_put(gbw_time_t *result,
                                            time_t input)
{
    *result = (gbw_time_t)input * GBW_USEC_PER_SEC;
    return 0;
}
...
gbw_time_t gbw_time_now(void)
{
    ...
    return tv.tv_sec * GBW_USEC_PER_SEC + tv.tv_usec;
}

static void explode_time(gbw_time_exp_t *xt, gbw_time_t t,
                         int32_t offset, int use_localtime)
{
    struct tm tm;
    time_t tt = (t / GBW_USEC_PER_SEC) + offset;
    xt->tm_usec = t % GBW_USEC_PER_SEC;
...
int gbw_time_exp_gmt(gbw_time_exp_t *result,
                                           gbw_time_t input)
{
    return gbw_time_exp_tz(result, input, 0);
}
                                                ...
                                                gbw_time_t input)
{
    ...
    return gbw_time_exp_tz(result, input, -timezone);
#else
...
int gbw_time_exp_get(gbw_time_t *t, gbw_time_exp_t *xt)
{
    gbw_time_t year = xt->tm_year;
    gbw_time_t days;
    ...
    *t = days * GBW_USEC_PER_SEC + xt->tm_usec;
    return 0;
}

int gbw_time_exp_gmt_get(gbw_time_t *t, 
                                               gbw_time_exp_t *xt)
{
    int status = gbw_time_exp_get(t, xt);
    if (status == 0)
        *t -= (gbw_time_t) xt->tm_gmtoff * GBW_USEC_PER_SEC;
    return status;
}
...
{
    struct timeval tv;
    tv.tv_usec = t % GBW_USEC_PER_SEC;
    tv.tv_sec = t / GBW_USEC_PER_SEC;
    select(0, NULL, NULL, NULL, &tv);
}


void gbw_unix_setup_time(void)
{
...
#if GBW_HAS_THREADS && defined(_POSIX_THREAD_SAFE_FUNCTIONS)
    gmtime_r(&t1, &t);
#else
    t = *gmtime(&t1);
#endif
