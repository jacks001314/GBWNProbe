...
static int file_read_buffered(gbw_file_t *thefile, void *buf,
                                       size_t *nbytes)
{
    ssize_t rv;
    char *pos = (char *)buf;
    ...
    if (thefile->direction == 1) {
        rv = gbw_file_flush_locked(thefile);
        if (rv) {
            return rv;
        }
        thefile->bufpos = 0;
...
int gbw_file_read(gbw_file_t *thefile, void *buf, size_t *nbytes)
{
    ssize_t rv;
    size_t bytes_read;
...
int gbw_file_write(gbw_file_t *thefile, const void *buf, size_t *nbytes)
{
    size_t rv;
...
int gbw_file_writev(gbw_file_t *thefile, const struct iovec *vec,
...
int gbw_file_putc(char ch, gbw_file_t *thefile)
{
    ...
    return gbw_file_write(thefile, &ch, &nbytes);
}
...
int gbw_file_getc(char *ch, gbw_file_t *thefile)
{
    ...
    return gbw_file_read(thefile, ch, &nbytes);
}

int gbw_file_puts(const char *str, gbw_file_t *thefile)
{
    return gbw_file_write_full(thefile, str, strlen(str), NULL);
}
...
int gbw_file_gets(char *str, int len, gbw_file_t *thefile)
{
     ...
     * and skip over the gbw_file_read calls.
     */
    if (thefile->buffered) {
        ...
        while (str < final) { /* leave room for trailing '\0' */
            /* Force ungetc leftover to call gbw_file_read. */
            if (thefile->bufpos < thefile->dataRead &&
                thefile->ungetchar == -1) {
        ...
        while (str < final) { /* leave room for trailing '\0' */
            nbytes = 1;
            rv = gbw_file_read(thefile, str, &nbytes);
            if (rv != 0) {
                break;
            }
            if (*str == '\n') {
...
struct gbw_file_printf_data {
    gbw_vformatter_buff_t vbuff;
    gbw_file_t *fptr;
    char *buf;
};

static int file_printf_flush(gbw_vformatter_buff_t *buff)
{
    struct gbw_file_printf_data *data = (struct gbw_file_printf_data *)buff;

    if (gbw_file_write_full(data->fptr, data->buf, 
...
int gbw_file_printf(gbw_file_t *fptr, 
                                        const char *format, ...)
{
    struct gbw_file_printf_data data;
    va_list ap;
    int count;
    ...
    data.fptr = fptr;
    va_start(ap, format);
    count = gbw_vformatter(file_printf_flush,
                           (gbw_vformatter_buff_t *)&data, format, ap);
    /* gbw_vformatter does not call flush for the last bits */
    if (count >= 0) file_printf_flush((gbw_vformatter_buff_t *)&data);
