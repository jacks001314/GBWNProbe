 ...
 */
int gbw_filepath_get(char **defpath, int32_t flags,
                                           gbw_pool_t *p)
{
	flags = flags;
    char path[GBW_PATH_MAX];

    if (!getcwd(path, sizeof(path))) {
        if (errno == ERANGE)
            return GBW_ENAMETOOLONG;
        else
            return errno;
    }
    *defpath = gbw_pstrdup(p, path);
                                             ...
                                             gbw_pool_t *p)
{
    ...
    if (addpath[0] == '/') {
         ...
         * Ths violates any GBW_FILEPATH_SECUREROOTTEST and
         * GBW_FILEPATH_NOTABSOLUTE flags specified.
         */
        if (flags & GBW_FILEPATH_SECUREROOTTEST)
            return GBW_EABOVEROOT;
        if (flags & GBW_FILEPATH_NOTABSOLUTE)
            return GBW_EABSOLUTE;

        /* If GBW_FILEPATH_NOTABOVEROOT wasn't specified,
         * we won't test the root again, it's ignored.
         * Waste no CPU retrieving the working path.
         */
        if (!rootpath && !(flags & GBW_FILEPATH_NOTABOVEROOT))
            rootpath = "";
    }
    else {
        /* If GBW_FILEPATH_NOTABSOLUTE is specified, the caller
    ...
    if (!rootpath) {
         ...
         */
        char *getpath;
        rv = gbw_filepath_get(&getpath, flags, p);
        rootpath = getpath;
        if (rv != 0)
            return errno;
         ...
         * Simplest would be a recursive call to gbw_filepath_merge
    ...
    maxlen = rootlen + strlen(addpath) + 4; /* 4 for slashes at start, after
                                             ...
                                             * null */
    if (maxlen > GBW_PATH_MAX) {
        return GBW_ENAMETOOLONG;
    }
    path = (char *)gbw_palloc(p, maxlen);
    ...
    if (addpath[0] == '/') {
         ...
         */
        if (rootpath[0] != '/' && (flags & GBW_FILEPATH_NOTRELATIVE))
            return GBW_ERELATIVE;
    ...
    while (*addpath) {
        ...
        if (seglen == 0 || (seglen == 1 && addpath[0] == '.')) {
            ...
            if (pathlen == 1 && path[0] == '/') {
                 ...
                 * GBW_FILEPATH_SECUREROOTTEST flag is specified.
                 */
                if (flags & GBW_FILEPATH_SECUREROOTTEST) {
                    return GBW_EABOVEROOT;
                }
                 ...
                 * GBW_FILEPATH_SECUREROOTTEST.was given die now.
                 */
                if (flags & GBW_FILEPATH_SECUREROOTTEST) {
                    return GBW_EABOVEROOT;
                }
     ...
     * backpath elements.  If so, and GBW_FILEPATH_NOTABOVEROOT
     * is specified (GBW_FILEPATH_SECUREROOTTEST was caught above),
     * compare the original root to assure the result path is
     * still within given root path.
     */
    if ((flags & GBW_FILEPATH_NOTABOVEROOT) && keptlen < rootlen) {
...
int gbw_filepath_list_split_impl(gbw_array_header_t **pathelts,
                                          const char *liststr,
                                          char separator,
                                          gbw_pool_t *p)
{
    ...
    elts = gbw_array_make(p, nelts, sizeof(char*));
    while ((part = gbw_strtok(path, separator_string, &ptr)) != NULL)
    {
        if (*part == '\0')      /* Ignore empty path components. */
            continue;

        *(char**)gbw_array_push(elts) = part;
        path = NULL;            /* For the next call to gbw_strtok */
    }
...
int gbw_filepath_list_merge_impl(char **liststr,
                                          ...
                                          gbw_pool_t *p)
{
    ...
    path = *liststr = gbw_palloc(p, path_size + 1);
    for (i = 0; i < pathelts->nelts; ++i)
    {
        /* ### Hmmmm. Calling strlen twice on the same string. Yuck.
               But is is better than reallocation in gbw_pstrcat? */
...
int gbw_filepath_list_split(gbw_array_header_t **pathelts,
                                                  ...
                                                  gbw_pool_t *p)
{
    return gbw_filepath_list_split_impl(pathelts, liststr, ':', p);
}
...
int gbw_filepath_list_merge(char **liststr,
                                                  ...
                                                  gbw_pool_t *p)
{
    return gbw_filepath_list_merge_impl(liststr, pathelts, ':', p);
}

int gbw_filepath_encoding(int *style, gbw_pool_t *p)
{
	p = p;
    *style = GBW_FILEPATH_ENCODING_LOCALE;
    return 0;
}
