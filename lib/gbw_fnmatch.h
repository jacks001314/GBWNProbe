...
#ifndef	GBW_FNMATCH_H
#define	GBW_FNMATCH_H

#include "gbw_mpool.h"
#include "gbw_tables.h"
/**
 * @defgroup gbw_fnmatch Filename Matching Functions
 * @ingroup GW 
 * @{
 */

#define GBW_FNM_NOMATCH     1     /**< Match failed. */
 
#define GBW_FNM_NOESCAPE    0x01  /**< Disable backslash escaping. */
#define GBW_FNM_PATHNAME    0x02  /**< Slash must be matched by slash. */
#define GBW_FNM_PERIOD      0x04  /**< Period must be matched by period. */
#define GBW_FNM_CASE_BLIND  0x08  /**< Compare characters case-insensitively. */

/**
 * Try to match the string to the given pattern, return 0 if
 *    match, else return GBW_FNM_NOMATCH.  Note that there is no such thing as
 *    an illegal pattern.
 *
 ...
 * <pre>
 *              GBW_FNM_NOESCAPE       Disable backslash escaping
 *              GBW_FNM_PATHNAME       Slash must be matched by slash
 *              GBW_FNM_PERIOD         Period must be matched by period
 *              GBW_FNM_CASE_BLIND     Compare characters case-insensitively.
 * </pre>
 */

int gbw_fnmatch(const char *pattern,const char *strings, int flags);
 ...
 */
int gbw_fnmatch_test(const char *pattern);
 ...
 * returned.
 */
int gbw_match_glob(const char *dir_pattern, 
                                         gbw_array_header_t **result,
                                         gbw_pool_t *p);
