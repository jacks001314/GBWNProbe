...
char * gbw_cpystrn(char *dst, const char *src, size_t dst_size)
{
 ...
 * be expanded to include support for the gbw_call_exec command line
...
int gbw_tokenize_to_argv(const char *arg_str, 
                                            char ***argv_out,
...
#define REMOVE_ESCAPE_CHARS(cleaned, dirty, escaped) \
    ...
    *argv_out = gbw_palloc(token_context, numargs * sizeof(char*));
    ...
    for (argnum = 0; argnum < (numargs-1); argnum++) {
        ...
        cp++;
        (*argv_out)[argnum] = gbw_palloc(token_context, cp - ct);
        gbw_cpystrn((*argv_out)[argnum], ct, cp - ct);
...
const char * gbw_filepath_name_get(const char *pathname)
{
 ...
 */
char * gbw_collapse_spaces(char *dest, const char *src)
{
    while (*src) {
        if (!gbw_isspace(*src)) 
            *dest++ = *src;
