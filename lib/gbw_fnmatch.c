...
static gbw_inline int fnmatch_ch(const char **pattern, const char **string, int flags)
{
    const char * const mismatch = *pattern;
    const int nocase = !!(flags & GBW_FNM_CASE_BLIND);
    const int escape = !(flags & GBW_FNM_NOESCAPE);
    const int slash = !!(flags & GBW_FNM_PATHNAME);
    int result = GBW_FNM_NOMATCH;
    const char *startch;
    int negate;
...
leadingclosebrace:
         ...
         */
        *pattern = mismatch;
        result = GBW_FNM_NOMATCH;
    }
    else if (**pattern == '?') {
        /* Optimize '?' match before unescaping **pattern */
        if (!**string || (slash && (**string == '/')))
            return GBW_FNM_NOMATCH;
        result = 0;
        goto fnmatch_gbw_success;
    }
...
int gbw_fnmatch(const char *pattern, const char *string, int flags)
{
...
firstsegment:
         ...
         */
        if ((flags & GBW_FNM_PERIOD) && (*string == '.'))
        {
            if (*pattern == '.')
                ++pattern;
            else if (escape && (*pattern == '\\') && (pattern[1] == '.'))
 ...
 * Solution may be to use fnmatgbw_ch() to walk the patterns?
 */
...
int gbw_match_glob(const char *pattern, 
                                         ...
                                         gbw_pool_t *p)
{
     ...
     * we really need gbw_filepath_basename, which will be coming as soon as
     * I get to it.  rbb
     */
    ...
    if (idx == NULL) {
        ...
        path = gbw_pstrndup(p, pattern, idx - pattern);
        pattern = idx + 1;
    }

    *result = gbw_array_make(p, 0, sizeof(char *));
    dir = opendir(path);
    ...
    while ((next = readdir(dir)) != NULL) {
        if (gbw_fnmatch(pattern, next->d_name, 0) == 0) {
            *(const char **)gbw_array_push(*result) = gbw_pstrdup(p, next->d_name);
        }
    }
