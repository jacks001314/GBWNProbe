/*
 *
 *      Filename: gbw_snprintf.c
 *
 *        Author: shajf,csp001314@163.com
 *   Description: ---
 *        Create: 2017-01-09 14:50:40
 * Last Modified: 2017-01-11 14:08:50
 */
...
static char *gbw_cvt(double arg, int ndigits, int *decpt, int *sign, 
                     int eflag, char *buf)
{
    register int r2;
    double fi, fj;
...
static char *gbw_ecvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (gbw_cvt(arg, ndigits, decpt, sign, 1, buf));
}

static char *gbw_fcvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (gbw_cvt(arg, ndigits, decpt, sign, 0, buf));
}

/*
 * gbw_gcvt  - Floating output conversion to
 * minimal length string
 */

static char *gbw_gcvt(double number, int ndigit, char *buf, boolean_e altform)
{
    int sign, decpt;
    register char *p1, *p2;
    register int i;
    ...
    p1 = gbw_ecvt(number, ndigit, &decpt, &sign, buf1);
    p2 = buf;
    if (sign)
        *p2++ = '-';
...
static char *conv_in_addr(struct in_addr *ia, char *buf_end, size_t *len)
{
    ...
    char *buf, size_t *len)
{
    ...
    if (format == 'f')
        p = gbw_fcvt(num, precision, &decimal_point, is_negative, buf1);
    else /* either e or E format */
        p = gbw_ecvt(num, precision + 1, &decimal_point, is_negative, buf1);
 ...
 */
int gbw_vformatter(int (*flush_func)(gbw_vformatter_buff_t *),
    gbw_vformatter_buff_t *vbuff, const char *fmt, va_list ap)
{
    ...
    while (*fmt) {
        if (*fmt != '%') {
            ...
            /*
             * Modifier check.  In same cases, GBW_OFF_T_FMT can be
             * "lld" and GBW_INT64_T_FMT can be "ld" (that is, off_t is
             * "larger" than int64). Check that case 1st.
             * Note that if GBW_OFF_T_FMT is "d",
             * the first if condition is never true. If GBW_INT64_T_FMT
             * is "d' then the second if condition is never true.
             */
            if ((sizeof(GBW_OFF_T_FMT) > sizeof(GBW_INT64_T_FMT)) &&
                ((sizeof(GBW_OFF_T_FMT) == 4 &&
                 fmt[0] == GBW_OFF_T_FMT[0] &&
                 fmt[1] == GBW_OFF_T_FMT[1]) ||
                (sizeof(GBW_OFF_T_FMT) == 3 &&
                 fmt[0] == GBW_OFF_T_FMT[0]) ||
                (sizeof(GBW_OFF_T_FMT) > 4 &&
                 strncmp(fmt, GBW_OFF_T_FMT, 
                         sizeof(GBW_OFF_T_FMT) - 2) == 0))) {
                /* Need to account for trailing 'd' and null in sizeof() */
                var_type = IS_QUAD;
                fmt += (sizeof(GBW_OFF_T_FMT) - 2);
            }
            else if ((sizeof(GBW_INT64_T_FMT) == 4 &&
                 fmt[0] == GBW_INT64_T_FMT[0] &&
                 fmt[1] == GBW_INT64_T_FMT[1]) ||
                (sizeof(GBW_INT64_T_FMT) == 3 &&
                 fmt[0] == GBW_INT64_T_FMT[0]) ||
                (sizeof(GBW_INT64_T_FMT) > 4 &&
                 strncmp(fmt, GBW_INT64_T_FMT, 
                         sizeof(GBW_INT64_T_FMT) - 2) == 0)) {
                /* Need to account for trailing 'd' and null in sizeof() */
                var_type = IS_QUAD;
                fmt += (sizeof(GBW_INT64_T_FMT) - 2);
            }
            else if (*fmt == 'q') {
                var_type = IS_QUAD;
                fmt++;
            ...
            case 'G':
                 ...
                 * * We use &num_buf[ 1 ], so that we have room for the sign
                 */
                s = gbw_gcvt(va_arg(ap, double), (int) precision, &num_buf[1],
                            alternate_form);
                if (*s == '-')
                    prefix_char = *s++;
                else if (print_sign)
            ...
            case 'p':
                ...
                case 'p':
                    if (sizeof(void *) <= sizeof(uint64_t)) {
                        ui_quad = (uint64_t) va_arg(ap, void *);
...
static int snprintf_flush(gbw_vformatter_buff_t *vbuff)
{
    /* if the buffer fills we have to abort immediately, there is no way
     * to "flush" an gbw_snprintf... there's nowhere to flush it to.
     */
	vbuff = vbuff;
...
int gbw_snprintf(char *buf, size_t len, 
                                     const char *format, ...)
{
    int cc;
    va_list ap;
    gbw_vformatter_buff_t vbuff;
    ...
    va_start(ap, format);
    cc = gbw_vformatter(snprintf_flush, &vbuff, format, ap);
    va_end(ap);
    if (len != 0) {
        *vbuff.curpos = '\0';
    }
...
int gbw_vsnprintf(char *buf, size_t len, const char *format,
                               va_list ap)
{
    int cc;
    gbw_vformatter_buff_t vbuff;
...
static int psprintf_flush(gbw_vformatter_buff_t *vbuff)
{
...
char * gbw_pvsprintf(gbw_pool_t *pool, const char *fmt, va_list ap)
{
    ...
    if (gbw_vformatter(psprintf_flush, &ps.vbuff, fmt, ap) == -1) {
