...
static inline int _new_memory(gbw_object_pool_t *omp){

	gbw_object_mem_t *objm;
	gbw_object_item_t *item;
	...
	objm = (gbw_object_mem_t*)addr;

	pos = (void*)(objm+1);
	end = addr+OBJECT_MEM_SIZE;

	/*alloc object item and add to free list*/
	r_item_size = omp->object_size+sizeof(gbw_object_item_t);

	while(pos+r_item_size<end){
	

		item = (gbw_object_item_t*)pos;
...
gbw_object_pool_t * gbw_object_pool_create(gbw_pool_t *mp,size_t object_limits,
	size_t object_size,
	void *priv_data,
	...
	if(object_size>=OBJECT_MEM_SIZE){
		...
		gbw_log(GBW_LOG_ERR,"The object size is too large:%lu",object_size);
		return NULL;
	}

	gbw_object_pool_t *objp = (gbw_object_pool_t *) gbw_pcalloc(mp,sizeof(*objp));
...
void gbw_object_pool_destroy(gbw_object_pool_t *omp){

	gbw_object_mem_t *objm = omp->mm_list;
	gbw_object_mem_t *prev;
...
void * gbw_object_pool_get(gbw_object_pool_t *omp){
	...
	if(list_empty(&omp->free_list)){
		...
		if(omp->object_limits&&omp->n_objects>omp->object_limits){
			...
			gbw_log(GBW_LOG_ERR,"Too many object has been alloced!");
			return NULL;
		}

		if(_new_memory(omp)){

			gbw_log(GBW_LOG_ERR,"No memory space to  new object!");
			return NULL;
		}
	}

	item = list_first_entry(&omp->free_list,gbw_object_item_t,node);
	list_del(&item->node);
...
void  gbw_object_pool_put(gbw_object_pool_t *omp,void *obj){

	if(obj == NULL)
		return;

	gbw_object_item_t *item = (gbw_object_item_t*)(obj-sizeof(*item));
...
#define ROBJ_SIZE(omp,n) (unsigned long)(((omp)->object_size+sizeof(gbw_object_item_t))*(n))

void gbw_object_pool_dump(gbw_object_pool_t *omp,FILE *fp){


	fprintf(fp,"Dump object pool info-------------------------------------------\n");
	
	fprintf(fp,"The object size:%lu\n",(unsigned long)(omp->object_size+sizeof(gbw_object_item_t)));
