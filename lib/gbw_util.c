 ...
 */
char *gbw_utf8_unicode_inplace_ex(gbw_pool_t *mp, unsigned char *input, long int input_len, int *changed) {
    int unicode_len = 0;
        ...
        /* If first byte begins with binary 110 it is two byte encoding*/
            ...
            /* check second byte starts with binary 10 */
                ...
                if(count <= len) {
                    ...
                    *data++ = '%';
                    *data++ = 'u';
                    unicode = (unsigned char*)gbw_psprintf(mp, "%x", d);
 ...
 */
unsigned char gbw_is_netmask_v4(char *ip_strv4) {
 ...
 */
unsigned char gbw_is_netmask_v6(char *ip_strv6) {
 ...
 */
char *gbw_remove_quotes(gbw_pool_t *mptmp, const char *input, int input_len)  {
    char *parm = gbw_palloc(mptmp, input_len);
    char *ret = parm;
    int len = input_len;
 ...
 * \retval string On Success
 */
char *gbw_remove_escape(gbw_pool_t *mptmp, const char *input, int input_len)  {
    char *parm = gbw_palloc(mptmp, input_len);
    char *ret = parm;
    int len = input_len;
 ...
 *
 */
int gbw_parse_boolean(const char *input) {
 ...
 */
int gbw_decode_base64_ext(char *plain_text, const unsigned char *input, int input_len)   {
 ...
 */
int gbw_convert_to_int(const char c)
{
    int n;
    if ((c>='0') && (c<='9'))
        n = c - '0';
 ...
 */
int gbw_parse_name_eq_value(gbw_pool_t *mp, const char *input, char **name, char **value) {
    char *p = NULL;
    ...
    *name = gbw_pstrmemdup(mp, input, p - input);
    if (*name == NULL) return -1;
    p++;

    *value = gbw_pstrdup(mp, p);
 ...
 */
char *gbw_url_encode(gbw_pool_t *mp, char *input, unsigned int input_len, int *changed) {
    char *rval, *d;
    unsigned int i, len;
 ...
 *
 */
char *gbw_file_basename(gbw_pool_t *mp, const char *filename) {
    char *d, *p;

    if (filename == NULL) return NULL;
    d = gbw_pstrdup(mp, filename);
...
char *gbw_file_dirname(gbw_pool_t *p, const char *filename) {
    char *b, *c;

    if (filename == NULL) return NULL;
    b = gbw_pstrdup(p, filename);
 ...
 *
 */
int gbw_sql_hex2bytes_inplace(unsigned char *data, int len) {
 ...
 *
 */
int gbw_hex2bytes_inplace(unsigned char *data, int len) {
 ...
 * representation.
 */
char *gbw_bytes2hex(gbw_pool_t *pool, unsigned char *data, int len) {
    static const unsigned char b2hex[] = "0123456789abcdef";
    char *hex = NULL;
    int i, j;

    hex = gbw_palloc(pool, (len * 2) + 1);
 ...
 * representation.
 */
char *gbw_bytes2hex_malloc(unsigned char *data, int len) {
    static const unsigned char b2hex[] = "0123456789abcdef";
    char *hex = NULL;
    int i, j;
 ...
 *
 */
int gbw_is_token_char(unsigned char c) {
 ...
 *
 */
int gbw_remove_lf_crlf_inplace(char *text) {
    char *p = text;
    int count = 0;
 ...
 *
 */
char *gbw_guess_tmp_dir(gbw_pool_t *p) {
    ...
    /* ENH Use gbw_temp_dir_get instead. */
 ...
 *
 */
char *gbw_current_logtime(gbw_pool_t *mp) {
    gbw_time_exp_t t;
    char tstr[100];
    size_t len;

    gbw_time_exp_lt(&t, gbw_time_now());

    gbw_strftime(tstr, &len, 80, "%d/%b/%Y:%H:%M:%S ", &t);
    gbw_snprintf(tstr + strlen(tstr), 80 - strlen(tstr), "%c%.2d%.2d",
            t.tm_gmtoff < 0 ? '-' : '+',
            t.tm_gmtoff / (60 * 60), (t.tm_gmtoff / 60) % 60);
    return gbw_pstrdup(mp, tstr);
}

char  *gbw_current_logtime_with_buf(char *buf,size_t buf_size){
    ...
    gbw_strftime(buf, &len, 80, "%d/%b/%Y:%H:%M:%S ", &t);

    gbw_snprintf(buf + strlen(buf), 80 - strlen(buf), "%c%.2d%.2d",
            t.tm_gmtoff < 0 ? '-' : '+',
            t.tm_gmtoff / (60 * 60), (t.tm_gmtoff / 60) % 60);
 ...
 *
 */
char *gbw_current_filetime(gbw_pool_t *mp) {
    gbw_time_exp_t t;
    char tstr[100];
    size_t len;
    ...
    gbw_strftime(tstr, &len, 80, "%Y%m%d-%H%M%S", &t);
    return gbw_pstrdup(mp, tstr);
}
 ...
 *
 */
int gbw_mkstemp_ex(char *templat, int mode) {
    int fd = -1;

    /* ENH Use gbw_file_mktemp instead. */
 ...
 *
 */
int gbw_mkstemp(char *templat) {
    return gbw_mkstemp_ex(templat, CREATEMODE_UNISTD);
}

/**
 * Converts the input string to lowercase (in-place).
 */
char *gbw_strtolower_inplace(unsigned char *str) {
    unsigned char *c = str;
...
static char *_log_escape(gbw_pool_t *p, const unsigned char *input,
        unsigned long int input_length, int escape_quotes, int escape_colon, int escape_re);

char *gbw_log_escape_re(gbw_pool_t *mp, const char *text) {
    return _log_escape(mp, (const unsigned char *)text, text ? strlen(text) : 0, 1, 1, 1);
}

char *gbw_log_escape(gbw_pool_t *mp, const char *text) {
    return _log_escape(mp, (const unsigned char *)text, text ? strlen(text) : 0, 1, 0, 0);
}

char *gbw_log_escape_nq(gbw_pool_t *mp, const char *text) {
    return _log_escape(mp, (const unsigned char *)text, text ? strlen(text) : 0, 0, 0, 0);
}

char *gbw_log_escape_ex(gbw_pool_t *mp, const char *text, unsigned long int text_length) {
    return _log_escape(mp, (const unsigned char *)text, text_length, 1, 0, 0);
}

char *gbw_log_escape_nq_ex(gbw_pool_t *mp, const char *text, unsigned long int text_length) {
    return _log_escape(mp, (const unsigned char *)text, text_length, 0, 0, 0);
}

char *gbw_log_escape_raw(gbw_pool_t *mp, const unsigned char *text, unsigned long int text_length) {
    unsigned char *ret = gbw_palloc(mp, text_length * 4 + 1);
    unsigned long int i, j;

    for (i = 0, j = 0; i < text_length; i++, j += 4) {
        ret[j] = '\\';
        ret[j+1] = 'x';
        gbw_c2x(text[i], ret+j+2);
    }
    ret[text_length * 4] = '\0';
...
char *gbw_log_escape_nul(gbw_pool_t *mp, const unsigned char *text, unsigned long int text_length) {
    unsigned char *ret = gbw_palloc(mp, text_length * 4 + 1);
    unsigned long int i, j;
 ...
 */
char *gbw_log_escape_hex(gbw_pool_t *mp, const unsigned char *text, unsigned long int text_length) {
    unsigned char *ret = gbw_palloc(mp, text_length * 4 + 1);
    unsigned long int i, j;
 ...
 */
static char *_log_escape(gbw_pool_t *mp, const unsigned char *input, unsigned long int input_len,
        int escape_quotes, int escape_colon, int escape_re)
{
    ...
    ret = gbw_palloc(mp, input_len * 4 + 1);
...
int gbw_js_decode_nonstrict_inplace(unsigned char *input, long int input_len) {
 ...
 * IMP1 Assumes NUL-terminated
 */
int gbw_urldecode_nonstrict_inplace_ex(unsigned char *input, long int input_len, int *invalid_count, int *changed) {
    unsigned char *d = (unsigned char *)input;
    long int i, count;
 ...
 * IMP1 Assumes NUL-terminated
 */
int gbw_html_entities_decode_inplace(gbw_pool_t *mp, unsigned char *input, int input_len) {
            ...
            if (input[j] == '#') {
                ...
                if ((input[j] == 'x')||(input[j] == 'X')) {
                    ...
                    if (j > k) { /* Do we have at least one digit? */
                        /* Decode the entity. */
                        char *x = gbw_pstrmemdup(mp, (const char *)&input[k], j - k);
 ...
 * IMP1 Assumes NUL-terminated
 */
int gbw_ansi_c_sequences_decode_inplace(unsigned char *input, int input_len) {
    unsigned char *d = input;
    int i, count;
 ...
 * IMP1 Assumes NUL-terminated
 */
int gbw_normalize_path_inplace(unsigned char *input, int input_len, int win, int *changed) {
    unsigned char *src;
    unsigned char *dst;
    unsigned char *end;
    int ldst = 0;
...
char *gbw_resolve_relative_path(gbw_pool_t *pool, const char *parent_filename, const char *filename) {
    if (filename == NULL) return NULL;
    // TODO Support paths on operating systems other than Unix.
    if (filename[0] == '/') return (char *)filename;

    return gbw_pstrcat(pool, gbw_pstrndup(pool, parent_filename,
                strlen(parent_filename) - strlen(gbw_filepath_name_get(parent_filename))),
            filename, NULL);
}
 ...
 */
int gbw_css_decode_inplace(unsigned char *input, long int input_len) {
    unsigned char *d = (unsigned char *)input;
    long int i, j, count;
            ...
            if (i + 1 < input_len) {
                ...
                if (j > 0) { /* We have at least one valid hexadecimal character. */
                    ...
                    switch (j) {
                        ...
                        case 1:
                            *d++ = gbw_xsingle2c(&input[i]);
                            break;

                        case 2:
                        case 3:
                            /* Use the last two from the end. */
                            *d++ = gbw_x2c(&input[i + j - 2]);
                            break;
...
/**
 * @brief Transforms an gbw_array_header_t to a text buffer
 *
 * Converts an gbw_array_header_t into a plain/text buffer in a Key: Pair
 ...
 *
 * @param arr gbw_array_header_t to be iterated.
 ...
 *
 */
int gbw_headers_to_buffer(const gbw_array_header_t *arr, char *buffer,
        int buffer_length)
{
    int headers_length = 0;
    int write_to_buffer = 0;
    int i = 0;
    const gbw_table_entry_t *te = NULL;

    if (buffer != NULL && buffer_length > 0) {
        write_to_buffer = 1;
    }

    te = (gbw_table_entry_t *)arr->elts;
...
int gbw_read_line(char *buf, int len, FILE *fp)
{
    char *tmp;
...
unsigned long gbw_get_current_timems(void)
{
   struct timeval tv;
...
int gbw_dir_make_r(const char *orig_path)
{
	char *s;
	char c;
	struct stat st;
...
unsigned char* gbw_encode_base64(gbw_pool_t *mp,const unsigned char *input,int input_len){

	size_t dst_len = BASE64_LEN(input_len);

	unsigned char *dst = (unsigned char*)gbw_pcalloc(mp,dst_len);
...
unsigned char* gbw_encode_base64_malloc(const unsigned char *input,int input_len){
