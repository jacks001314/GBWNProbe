                                      ...
                                      int argc, const char *const *argv)
{
    ...
    *os = gbw_palloc(cont, sizeof(gbw_getopt_t));
    (*os)->cont = cont;
    (*os)->reset = 0;
    (*os)->errfn = (gbw_getopt_err_fn_t*)(fprintf);
    ...
    argv_buff = gbw_palloc(cont, (argc + 1) * sizeof(const char *));
    ...
    if ((os->opt = (int) *os->place++) == (int) ':' ||
        ...
        if (os->errfn && *opts != ':') {
            (os->errfn)(os->errarg, "%s: illegal option -- %c\n",
                        gbw_filepath_name_get(*os->argv), os->opt);
        }
        *optch = os->opt;
        return (GBW_BADCH);
    }
 ...
 * is where we are now.
 */
static void permute(gbw_getopt_t *os)
{
...
static int serr(gbw_getopt_t *os, const char *err, const char *str,
                         int status)
{
    if (os->errfn)
        (os->errfn)(os->errarg, "%s: %s: %s\n", 
                    gbw_filepath_name_get(*os->argv), err, str);
    return status;
}

/* Helper function to print out an error involving a short option */
static int cerr(gbw_getopt_t *os, const char *err, int ch,
                         int status)
{
    if (os->errfn)
        (os->errfn)(os->errarg, "%s: %s: %c\n", 
                    gbw_filepath_name_get(*os->argv), err, ch);
    return status;
}
                                          ...
                                          int *optch, const char **optarg)
{
        ...
        if (*p == '-' && p[1] != '\0') {        /* Long option */
            ...
            for (i = 0; ; i++) {
                if (opts[i].optch == 0)             /* No match */
                    return serr(os, "invalid option", p - 2, GBW_BADCH);
            ...
            *optch = opts[i].optch;
            ...
            if (opts[i].has_arg) {
                if (p[len] == '=')             /* Argument inline */
                    *optarg = p + len + 1;
                    ...
                    if (os->ind >= os->argc)   /* Argument missing */
                        return serr(os, "missing argument", p - 2, GBW_BADARG);
                ...
                *optarg = NULL;
                if (p[len] == '=')
                    return serr(os, "erroneous argument", p - 2, GBW_BADARG);
            }
            permute(os);
            return 0;
        } else {
            if (*p == '-') {                 /* Bare "--"; we're done */
                ...
                if (*p == '\0')                    /* Bare "-" is illegal */
                    return serr(os, "invalid option", p, GBW_BADCH);
        }
    }
    ...
    for (i = 0; ; i++) {
        if (opts[i].optch == 0)                     /* No match */
            return cerr(os, "invalid option character", *p, GBW_BADCH);
    ...
    if (opts[i].has_arg) {
        if (*p != '\0')                         /* Argument inline */
            *optarg = p;
            ...
            if (os->ind >= os->argc)           /* Argument missing */
                return cerr(os, "missing argument", *optch, GBW_BADARG);
            else                               /* Argument in next arg */
                *optarg = os->argv[os->ind++];
        }
        os->place = EMSG;
